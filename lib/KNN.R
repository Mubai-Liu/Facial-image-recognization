...
Smarket1 <- data[c(data_index), ][1:2702700,c(1,3,4)]
Smarket2 <- data[c(data_index),][2702701:5405400,c(1,3,4)]
...
KNN <- as.numeric(KNN.decision())


accu <- mean(Smarket1$direction == Smarket2$direction)
cat("The accuracy of model:", model_labels[which.min(err_cv[,1])], "is", accu*100, "%.\n")

auc <- WeightedROC(KNN, Smarket1$direction) %>% WeightedAUC
cat("The AUC of model:", model_labels[which.min(err_cv[,1])], "is", auc, ".\n")


WeightedROC(as.numeric(Smarket2$direction), as.numeric(Smarket1$direction)) %>%
  WeightedAUC
accu*100
