...
auc <- WeightedROC(pred_gbm, dat_test_default$label, weight_test) %>% WeightedAUC # weighted
...
### evaluation
```{r}
accu.unweighted <- mean(dat_test$label == xgb_pred)
accu <- sum(weight_test_improved * (xgb_pred == dat_test$label))/sum(weight_test_improved)
